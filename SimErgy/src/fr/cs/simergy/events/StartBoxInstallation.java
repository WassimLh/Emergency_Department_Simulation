 ...
 * An L3, L4 or L5 patient is installed by a nurse in a Box Room
 * to get visited.
 */
public class StartBoxInstallation extends Event {
	//Attributes
	/**
	 ...
	 * @param sim
	 */
	public StartBoxInstallation(EmergencySimulation sim){
		this.sim = sim;
	}
	//Methods
	/**
	...
	@Override
	public void actions(){
		Patient patient = this.sim.getSort().getPriorityPatient(this.sim.getPatientsWaitingForBoxInstallation());
		...
		Room room = (Room) this.sim.getEmptyBoxRooms().first();
		this.room = room;
		room.out();
